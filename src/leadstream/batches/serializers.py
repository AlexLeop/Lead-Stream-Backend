...
class BatchSerializer(serializers.ModelSerializer[Batch]):
    ...
    class Meta:
        model = Batch
        fields = (
            "id", "name", "source_type", "status", "current_stage",
            "input_original_name", "input_size_bytes", "input_sha256", "chunk_size",
            "total_rows", "processed_rows", "succeeded_rows", "absent_rows", "failed_rows",
            "duplicate_rows", "corrected_rows", "invalid_rows", "progress_percent",
            "eta_seconds", "cost_cents", "revenue_cents", "gross_profit_cents",
    ...
    def get_gross_profit_cents(self, obj: Batch) -> int:
        return int(obj.revenue_cents) - int(obj.cost_cents)
...
class ProcessingAttemptSerializer(serializers.ModelSerializer[ProcessingAttempt]):
    class Meta:
        ...
        fields = (
            "id", "attempt_number", "worker_id", "status", "error_code", "error_message",
