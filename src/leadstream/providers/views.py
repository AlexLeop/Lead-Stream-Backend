    ...
    def post(self, request: Request, batch_id: UUID) -> Response:
        ...
        try:
            batch = start_batch_enrichment(
                tenant=resolve_tenant(request), batch_id=batch_id, requested_blocks=blocks
            )
...
def _get_discovery(search_id: UUID, request: Request) -> DiscoverySearch:
    try:
        return DiscoverySearch.objects.get(pk=search_id, tenant=resolve_tenant(request))
...
class DiscoveryCollectionView(APIView):
    ...
    def get(self, request: Request) -> Response:
        queryset = DiscoverySearch.objects.filter(tenant=resolve_tenant(request))
...
class DiscoveryDetailView(APIView):
    ...
    def get(self, request: Request, search_id: UUID) -> Response:
        return Response(DiscoverySearchSerializer(_get_discovery(search_id, request)).data)


class DiscoveryResultsView(APIView):
    @extend_schema(responses=DiscoveryResultSerializer(many=True), tags=["Descoberta"])
    def get(self, request: Request, search_id: UUID) -> Response:
        search = _get_discovery(search_id, request)
